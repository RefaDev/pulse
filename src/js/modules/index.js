...
const catalogItemHandler = () => {
  const items = document.querySelectorAll('.catalog-item__link')
  const backItems = document.querySelectorAll('.catalog-item__back')
  items.forEach((item) => {
  ...
  backItems.forEach((item) => {
    item.addEventListener('click', function (e) {
      e.preventDefault()
      e.target.parentElement.classList.toggle('catalog-item__list_active')
